...
axζo = Axis3(fig[1, 1]; kw...)
axua = Axis3(fig[1, 2]; kw...)
axTa = Axis3(fig[1, 3]; kw...)

sf = surface!(axζo, x, y, z, color=ζon, colorrange=(-5e-5, 5e-5), colormap=:balance, nan_color=:lightgray)
Colorbar(fig[2, 1], sf, width=Relative(0.6), vertical=false, label = "Ocean vorticity (s⁻¹)", labelsize=20)

sf = surface!(axua, x, y, z, color=uan, colorrange=(-40, 40), colormap=:balance, nan_color=:lightgray)
Colorbar(fig[2, 2], sf, width=Relative(0.6), vertical=false, label = "Atmosphere zonal velocity (m s⁻¹)", labelsize=20)

sf = surface!(axTa, x, y, z, color=Tan, colorrange=(270, 310), colormap=:magma, nan_color=:lightgray)
Colorbar(fig[2, 3], sf, width=Relative(0.6), vertical=false, label = "Atmosphere surface temperature (K)", labelsize=20)

t = uat.times
title = @lift @sprintf("%s days", round((t[$n]+1e-6) / Oceananigans.Units.day, digits=2))
Label(fig[0, :], title, fontsize=24)

hidedecorations!(axζo)
hidedecorations!(axua)
hidedecorations!(axTa)
hidespines!(axζo)
hidespines!(axua)
hidespines!(axTa)
